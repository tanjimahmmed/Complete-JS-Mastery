...
// 1.
dogs.forEach(dog => (dog.raceFood = dog.weight ** 0.75 * 28));
console.log(dogs);

// 2
const dogSarah = dogs.find(dog => dog.owners.includes('Sarah'));
console.log(`Sarah's dog eats too ${dogSarah.curFood ? 'much' : 'little'}`);

// 3
const ownersTooMuch = dogs.filter(dog => dog.curFood > dog.raceFood).flatMap(dog => dog.owners);
const ownersTooLittle = dogs.filter(dog => dog.curFood < dog.raceFood).flatMap(dog => dog.owners);
...
// 5
console.log(dogs.some(dog => dog.curFood === dog.raceFood));

// 6
const checkEatingOkay = dog => dog.curFood < dog.raceFood * 1.1 && dog.curFood > dog.raceFood * 0.9;
...
const dogsGroupedByPortion = Object.groupBy(dogs, dog => {
  if(dog.curFood > dog.raceFood){
    return 'too-much'
  }else if (dog.curFood < dog.raceFood){
    return 'too-little'
  }else {
    return 'exact'
  }
...
// 9
const dogGroupedByOwners = Object.groupBy(dogs, dog => `${dog.owners.length}-owners`);
console.log(dogGroupedByOwners);


// 10
const dogSorted = dogs.toSorted((a, b) => b.raceFood - a.raceFood);
